...
/**
 * A fragment that organizes activities in 4 Levels and displays them to the user. Created by klas on 06.10.13.
 */
public class ActivitiesOverviewListFragment extends BaseFragment {
	...
	private class Level1OverviewLoadingListAdapter extends BaseExpandableListAdapter implements LoaderManager.LoaderCallbacks<List<TrackedActivityModel>>, ExpandableListView.OnGroupClickListener, View.OnClickListener {
		...
		void updateItems() {
			...
			Iterable<TrackedActivityModel> activities = _filterPredicate != null ? Iterables.filter(_activities, _filterPredicate) : _activities;
	...
	public static final class OverviewTrackedActivitiesListAdapter extends TrackedActivitiesListAdapter implements View.OnClickListener {
		...
		protected View getOrCreateView(View convertView, ViewGroup parent, TrackedActivityModel item) {
			TrackedActivityModel oldItem = convertView == null ? null
					: (convertView.getTag() instanceof TrackedActivityModel ? (TrackedActivityModel)convertView.getTag() : null);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (itemId == R.id.action_add) {
			...
			final String title = "Aktivitäten " + DateUtils.formatDateTime(getActivity(), getStarttimeInclusiveMillis(), DateUtils.FORMAT_SHOW_DATE) + " - "
					+ DateUtils.formatDateTime(getActivity(), getEndtimeExclusiveMillis(), DateUtils.FORMAT_SHOW_DATE) + ".csv";
