...
public class TrackedActivityFragment extends AnalyticsBaseCRUDFragment<TrackedActivityFragmentView, TrackedActivityFragmentData> implements View.OnClickListener, AdapterView.OnItemSelectedListener, TimePickerSupport.LocalTimeSelectedListener, AutoCompleteSpinner.OnItemSetListener, DatePickerSupport.LocalDateSelectedListener {
	...
	protected void save() {
		...
		final InsertOrUpdateOperationFactory customFieldsOperationFactory = view.getCustomFieldListAdapter().getSaveOrUpdateOperationFactory();
	...
	public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
		...
		if (parentId == TrackedActivityFragmentView.ID_START_TIME) {
			onTimeSuggestionSelected(
				TrackedActivityFragmentView.ID_START_TIME,
				(TimeSuggestion)parent.getSelectedItem(),
				_data.getStartTime());
		}
		else if (parentId == TrackedActivityFragmentView.ID_END_TIME) {
			onTimeSuggestionSelected(
	...
	public void onTimeSelected(int token, LocalTime time) {
		if (token == TrackedActivityFragmentView.ID_START_TIME) {
		...
		else if (token == TrackedActivityFragmentView.ID_END_TIME) {
	...
	public void onItemSet(AutoCompleteSpinner spinner, boolean userSelectedExplicitely, int position, long itemId) {
		...
		if (spinnerId == TrackedActivityFragmentView.ID_PROJECT_SPINNER) {
			onProjectSpinnerItemSet(position, (CursorAdapter)spinner.getAdapter());
		}
		else if (spinnerId == TrackedActivityFragmentView.ID_ACTIVITY_SUGGESTION_SPINNER) {
	...
	private void onTimeSuggestionSelected(int token, TimeSuggestion suggestion, LocalTime currentValue) {
		TimeSuggestion.Type type = suggestion.getType();
		if (type == TimeSuggestion.Type.ANY_TIME) {
	...
	public void onClick(View v) {
		int viewId = v.getId();
		if (viewId == TrackedActivityFragmentView.ID_DAY_INPUT) {
			onChangeDayClicked();
			return;
		}
	}
	...
	public void onDateSelected(int token, final LocalDate date) {
		if (token == TrackedActivityFragmentView.ID_DAY_INPUT) {
	...
	/**
	 * To apply a selected day, we need to load the time suggestions for the day first. Asynchronously, of course.
	 */
	...
	private class ApplyActivityTypeEditTask extends AsyncTask<Long, Void, ActivityTypeSuggestionModel> {
		...
		protected ActivityTypeSuggestionModel doInBackground(Long... params) {
			...
			final ActivityTypeSuggestionSyncLoader suggestionSyncLoader = new ActivityTypeSuggestionSyncLoader(getContext(), projectSyncLoader,
					activityTypeSyncLoader);

			try {
				final Map<Long, ActivityTypeModel> modelMap = activityTypeSyncLoader.loadAsMap(MCContract.ActivityType._ID + "=?",
					new String[] {Long.toString(activityTypeId)}, MCContract.ActivityType._ID + " desc");
