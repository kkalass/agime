 ...
 */
public class ActionViewController implements View.OnClickListener, UndoBarController.UndoListener {
	...
	public void onClick(View v) {
		...
		if (viewId == ActionViewWrapper.ID_BUTTON1) {
	...
	public void onCloseActionItemHeading(ActionItem tag) {
		...
		final LocalTime acquisitionEndTime = (acquisitionEndDateTime != null && acquisitionEndDateTime.toLocalDate().equals(_date))
				? acquisitionEndDateTime.toLocalTime()
				: null;
	...
	private void bindNewEntryNeeded(ActionViewWrapper actionView, ActionItem actionItemData) {
		...
		actionView.button2
			.setText(Html.fromHtml(getString(R.string.action_item_of_day_extend_previous_action, previousTitle, Long.toString(actionItemData.getDurationMinutes()))));
		...
		if (isStartAcquisitionTime(actionItemData)) {
			...
			if (actionItemData.getDurationMinutes() < 5) {
				if (actionItemData._mode == ActionItem.Mode.START_OF_DAY) {
					actionView.details
						.setText(Html.fromHtml(getString(R.string.action_first_item_of_day_message_acquisition_time_started, formatEndTime(actionItemData))));
				}
				else {
					actionView.details.setText(Html.fromHtml(getString(R.string.action_first_item_of_day_message_no_minutes)));
				}
				...
				actionView.details.setText(Html.fromHtml(getString(R.string.action_first_item_of_day_message, Long.toString(actionItemData.getDurationMinutes()),
					TimeFormatUtil.formatTime(getContext(), actionItemData._startTimeMillis))));
			}
			actionView.button1.setText(R.string.action_item_of_day_action);
		}
	...
	private void bindEndOfDay(ActionViewWrapper actionView, ActionItem actionItemData) {
		...
		if (previousAcquisitionTimeUnfulfilled) {
			...
			actionView.details.setText(Html.fromHtml(String.format(text,
				...
				percentage)));
			actionView.headingText.setText(actionItemData._mode == ActionItem.Mode.END_OF_DAY ? getText(R.string.action_last_item_of_day_heading)
	...
	protected void reduceTitle(float scale) {
		...
		float headerTargetTranslationY = getDimensionPixelSize(R.dimen.chronicle_toolbar_header_margin_top_target)
				- getDimensionPixelSize(R.dimen.chronicle_toolbar_header_margin_top_initial);
