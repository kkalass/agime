...
/**
 * Pseudo-Preference: makes the list of custom field types embeddable in the preferences screen, including an "add"
 * button.
 *
 * Created by klas on 19.12.13.
 */
public class CustomFieldTypeListPreference extends CustomPreference implements View.OnClickListener, LinearListView.OnItemClickListener {
	...
	public void onClick(View v) {
		int viewId = v.getId();
		if (viewId == WrappedView.ID_INSERT_BUTTON) {
	...
	public void onItemClick(LinearListView linearListView, View view, int position, long l) {
		int listViewId = linearListView.getId();
		if (listViewId == WrappedView.ID_LIST) {
	...
	private static class CustomFieldTypeListAdapter extends BaseListAdapter<CustomFieldTypeWrappedView, CustomFieldTypeDAO.Data> implements View.OnClickListener {
		...
		/**
		 * The number of custom fields is cached in the preferences so that the UI contains the correct number of custom
		 * field entries right from the start.
		 *
		...
		public void onClick(View v) {
			int viewId = v.getId();
			if (viewId == CustomFieldTypeWrappedView.ID_EDIT) {
