...
public class AgimeMainActivity extends MainEntryPointActivity implements ResizableToolbarHelper.ResizableToolbarActivity {
		...
		public void onClick(View v) {
			switch (v.getId()) {
				...
				case R.id.action_faq:
					selectItem(v.getId(), POS_ACTIVITY_FAQ, null);
					return;
				case R.id.action_settings:
					selectItem(v.getId(), POS_ACTIVITY_SETTINGS, null);
					return;
				case R.id.action_acquisition_times:
					selectItem(v.getId(), POS_ACTIVITY_SETTINGS_ACQUISITION, null);
					return;
				case R.id.action_chronicle:
					selectItem(v.getId(), POSITION_CHRONICLE_BY_DAY, null);
					return;
				case R.id.action_report:
					selectItem(v.getId(), POSITION_REPORT, _reportType == null ? TYPE_REPORT_BY_DAY : _reportType);
					return;
			}
		}
	};
	...
	private Fragment createFragment(int position, Short typeReport) {
		...
		switch (position) {
			case POSITION_CHRONICLE_BY_DAY:
	...
	/**
	 * Prüft, ob die Berechtigung für Benachrichtigungen bereits vorhanden ist. Falls nicht, wird eine Anfrage für die
	 * Berechtigung gestellt.
	 */
	private void checkNotificationPermission() {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
			if (ContextCompat.checkSelfPermission(this, Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) {

				// Berechtigungsanfrage zeigen, wenn sie noch nicht gewährt wurde
	...
	public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
		...
		if (requestCode == REQUEST_NOTIFICATION_PERMISSION) {
			if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
				...
				Log.i(LOG_TAG, "Notification permission was denied");
				// Optional: Dem Benutzer erklären, warum Benachrichtigungen wichtig sind
				Toast.makeText(
					this,
					"Ohne Benachrichtigungsberechtigung werden Erinnerungen nicht angezeigt",
