	...
	private final class BackgroundService extends Handler {
		...
		private Notification createForegroundNotificationAndInstallTimer(
			...
			if (current != null) {
				...
				if (previous != null) {
					DateTime notificationRefreshTime = previous.getEndDateTime().plusMinutes(AcquisitionTimeInstance.ACQUISITION_TIME_END_THRESHOLD_MINUTES)
						.plusMillis(500);
					AlarmManagerSupport.setAlarm(am,
						AlarmManager.RTC_WAKEUP,
		...
		private Notification createForegroundNotification(AlarmManager am, AcquisitionTimes times, DateTime now, TrackedActivityModel lastActivity, Context context,
			...
			if (times.getCurrent() == null) {
				...
				if (lastActivity != null) {
					...
					builder.setContentText(
						getString(R.string.ongoing_notif_after_acquisition_without_previous_activity_content_text, TimeFormatUtil.formatTime(context, startTimeMillis)));
				}
				...
				builder.setUsesChronometer(true);
				builder
					.setContentText(getString(R.string.ongoing_notif_in_acquisition_first_activity_content_text, TimeFormatUtil.formatTime(context, startTimeMillis)));
				...
				if (millisSinceLast < UP_TO_DATE_MILLIS_SINCE_ACTIVITY) {
					...
					builder.setContentTitle(
						getString(R.string.ongoing_notif_in_acquisition_next_activity_within_threshold_content_title, lastActivity.getDisplayName(context)));
		...
		private long handleNoise(NotificationCompat.Builder builder, long startTimeMillis) {
			/**
			 * User Story: I as a User want Agime to notify me with an extra noise every XXX Minutes if I did not track my
			 * activity, so that I learn to always track my activities at least every XXX Minutes.
			 *
			 *
			 * Umsetzung: - Schedulen, dass dieser Service aufgerufen wird - hier gibt es ggf. Konflikte mit den anderen
			 * Alarm-Schedulings -> rückgabewert, in wie vielen Millis die nächste Benachrichtigung kommen soll. - Auslösen
			 * einer besonderen Benachrichtigung, wenn die Notification aktualisiert wird und die Bedingung eintritt.
			 *
			 * Ich verwende User Preferences, um den nächsten Noise-Punkt festzulegen. Wenn der Noise-Punkt erreicht oder
			 * überschritten wird, dann noise hinzufügen. Noise-Punkt ergibt sich aus dem Tupel "letzte startzeit" und "noise
			 * zeit", damit bei zwischenzeitlicher Veränderung die
			 */
